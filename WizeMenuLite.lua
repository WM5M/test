...
local function isKeyValid()
    ...
    -- Removed os.time() dependency; assume valid if key exists (all keys are unexpired as of Oct 14, 2025)
    -- local now = os.time()  -- Commented out
    for _, keyData in ipairs(keys) do
        if keyData.key == CurrentKey then
            -- Removed expiration check
            -- if keyData.expires then
            --     local year, month, day, hour, min, sec = string.match(keyData.expires, "([%d]+)-([%d]+)-([%d]+)T([%d]+):([%d]+):([%d]+)Z")
            --     if year and month and day and hour and min and sec then
            --         local expiresTime = os.time({ year=tonumber(year), month=tonumber(month), day=tonumber(day), hour=tonumber(hour), min=tonumber(min), sec=tonumber(sec) })
            --         if expiresTime > now then
            --             return true
            --         end
            --     end
            -- end
            return true  -- Key matches; treat as valid
        end
    end
        ...
        if ok and keys and type(keys) == "table" then
            for _, keyData in ipairs(keys) do
                if keyData.key == CurrentKey and keyData.expires then
                    ...
                    if year and month and day and hour and min and sec then
                        -- Removed os.time() here too; hardcoded a message assuming validity
                        -- local now = os.time()
                        -- local expiresTime = os.time({ year=tonumber(year), month=tonumber(month), day=tonumber(day), hour=tonumber(hour), min=tonumber(min), sec=tonumber(sec) })
                        ...
                        local expirationMessage = "Your key is valid. Enjoy!"  -- Simplified message
                        -- if remainingDays > 0 then
                        --     expirationMessage = string.format("You have %d days and %d hours left. Enjoy!", remainingDays, remainingHours)
                        -- else
...
local VIPTab = MachoMenuAddTab(MenuWindow, "Purchase VIP")
...
local function EventTabContent(tab)
    ...
    local SectionOne = MachoMenuGroup(tab, "Money Spawner", leftX, topY, leftX + ColumnWidth, topY + HalfHeight)
    --local SectionOne = MachoMenuGroup(tab, "Money Spawner", leftX, midY, leftX + ColumnWidth, midY + HalfHeight)
...
local function CheckResource(resource)
 ...
 local function LoadBypasses()
    ...
    MachoMenuNotification("[NOTIFICATION] WizeMenu", "Loading Spawn Bypass.")
    ...
    MachoMenuNotification("[NOTIFICATION] WizeMenu", "Loading WaveSHIT Bypass.")
...
MachoMenuButton(PlayerTabSections[3], "Revive VIP Only", function()
end)
...
MachoMenuButton(ServerTabSections[1], "Force Robbery", function()
    MachoInjectResource(
...
MachoMenuButton(ServerTabSections[2], "Crash [VIP Only]", function()
...
MachoMenuButton(VIPTabSections[1], "Gain VIP Access", function()
    ...
    if ItemName and ItemName ~= "" and ItemAmount and tonumber(ItemAmount) then
        ...
        -- Optionally run the action immediately, or handle it later:
        if GetResourceState("qb-uwujob") == "started" then
            resourceActions["qb-uwujob"]()
        else
            MachoMenuNotification("[NOTIFICATION] WizeMenu", "qb-uwujob not started.")
        end
    else
        MachoMenuNotification("[NOTIFICATION] WizeMenu", "Open Ticket In WizeMenu Discord.")
    end
end)
            ...
            MachoMenuButton(VIPTabSections[2], "Trigger Finder", function()
    if not HasValidKey() then return end

    if SomeLicenseCheck() then
        local resourceActions = {
            ["skirpz_drugplug"] = function()
                MachoInjectResource2(3,
                    (CheckResource("ReaperV4") and "ReaperV4")
                    or (CheckResource("monitor") and "monitor")
                    or "any",
                    [[
        ...
        MachoMenuNotification("[NOTIFICATION] WizeMenu", "Upgrade your license.")
    end
end)
...
MachoMenuButton(VIPTabSections[2], "Police Job", function()
    ...
    if CheckResource("wasabi_multijob") then
        ...
        MachoMenuNotification("[NOTIFICATION] WizeMenu", "Resource Not Found.")
    end
end)
...
MachoMenuButton(VIPTabSections[3], "Spoofed Weapon Bypass", function()
     ...
     local function awfawrwr3wsd()
             local awfawrwr3wsd = CreateThread
