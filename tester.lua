...
RegisterNUICallback('accessCodeSubmit', function(data, cb)
    if data.code == ACCESS_CODE then
        menuUnlocked = true
        SendNUIMessage({type = "hideAccessModal", show = false})
        MachoMenuNotification("Katana Menu", "✅ Access Granted! Welcome!", 5)
        
        -- NOW LOAD FULL MENU
        Citizen.SetTimeout(500, function()
            updateMenu()  -- Your main menu function
        end)
    else
        MachoMenuNotification("Katana Menu", "❌ Wrong Code! Try Again.", 3)
        document.getElementById('accessCodeInput').value = ''  -- Clear input
    end
    cb('ok')
end)

RegisterNUICallback('accessCodeClose', function(data, cb)
    menuUnlocked = false
    SendNUIMessage({type = "hideAccessModal", show = false})
    cb('ok')
end)
...
if not isKeyValid() then
    MachoMenuNotification("Katana Menu", "Your key ain't valid lmfao: " .. CurrentKey, 10)
    return
end

-- ========== SHOW ACCESS MODAL ==========
SendNUIMessage({
    type = "showAccessModal",
    show = true
})

-- ========== FULL MENU (ONLY AFTER UNLOCK) ==========
Citizen.CreateThread(function()
    while not menuUnlocked do
        Citizen.Wait(100)
    end
    
    -- Key expiration message
    local ok, keys = pcall(json.decode, KeysBin)
    if ok and keys and type(keys) == "table" then
        for _, keyData in ipairs(keys) do
            if keyData.key == CurrentKey then
                MachoMenuNotification("Katana Menu", "Your key is valid. Enjoy!", 5)
                break
            end
        end
    end
...
local function CheckResource(resource)
 ...
 local function LoadBypasses()
    Wait(1500)
    MachoMenuNotification("[NOTIFICATION] Katana Menu", "Loading Car Spawn Bypass.")

    MachoInjectResourceRaw('lb-phone', [[
    local function runLbPhoneBypass()
        CreateThread(function()
            print('[^1KATANA^0] ^3Loading lb-phone patch bypass...^0')
            ...
            while true do
                if lbPhoneCFV ~= _G.CreateFrameworkVehicle then
                    ...
                    print('[^1KATANA^0] ^2Loaded Katana Car Spawn bypass^0')
                end
    ...
    MachoMenuNotification("[NOTIFICATION] KATANA", "Loading WaveSHIT Bypass.")
    ...
    MachoMenuNotification("[NOTIFICATION] Katana Menu", "Bypassing FiveTard.")
    ...
    MachoMenuNotification("[NOTIFICATION] Katana Menu", "Finished Enjoy.")
end
...
-- Execute the main function
                    ...
                    MachoMenuNotification("Katana MeNu", "More Features Coming Soon..", 5)
                end}
            }
        },
